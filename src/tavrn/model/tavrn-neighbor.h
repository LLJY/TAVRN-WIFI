...
#ifndef TAVRN_NEIGHBOR_H
#define TAVRN_NEIGHBOR_H
 ...
 *
 * Extends the AODV neighbor management with per-neighbor RSSI caching
 * needed for TAVRN's mentorship backoff (stronger signal -> shorter wait).
 */
class Neighbors
{
  public:
    ...
    /// @name RSSI management for TAVRN mentorship backoff
    //\{
    /**
     * Get cached RSSI for a neighbor
     * @param neighbor the IP address of the neighbor
     * @returns the RSSI value in dBm, or 0.0 if not available
     */
    double GetRssi(Ipv4Address neighbor) const;
    /**
     * Set/update cached RSSI for a neighbor
     * @param neighbor the IP address of the neighbor
     * @param rssi the RSSI value in dBm
     */
    void SetRssi(Ipv4Address neighbor, double rssi);

    /**
     * Find MAC address by IP using list of ARP caches
     * made public so RoutingProtocol can resolve IP->MAC for RSSI lookup.
     *
    ...
    /**
     * Reverse lookup: find the IP address for a given MAC address from the
     * neighbor table. Must be called BEFORE ProcessTxError/Purge removes the entry.
     *
  ...
  private:
    ...
    /// Per-neighbor RSSI cache (dBm) for mentorship backoff calculation
    std::map<Ipv4Address, double> m_rssiCache;
